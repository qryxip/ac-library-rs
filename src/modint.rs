...
impl Barrett {
    ...
    fn store(&self, m: u32) {
        ...
        self.m.store(m, atomic::Ordering::Relaxed);
        self.im.store(im, atomic::Ordering::Relaxed);
    }

    #[inline]
    fn umod(&self) -> u32 {
        self.m.load(atomic::Ordering::Relaxed)
    }

    #[inline]
    fn mul(&self, a: u32, b: u32) -> u32 {
        let m = self.m.load(atomic::Ordering::Relaxed);
        let im = self.im.load(atomic::Ordering::Relaxed);
